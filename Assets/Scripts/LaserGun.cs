...
public class LaserGun : Tower
{
    ...
    [SerializeField] private float start_damage = 1;

    [SerializeField] private float max_damage = 150;

    [SerializeField] private int damage_interval = 3;

    [SerializeField] private float damage_coef = 1.5f;

    [SerializeField] private float rotation_speed = 10;

    [SerializeField] private GameObject target_fire;

    [SerializeField] private Transform laser;

    [SerializeField] private Transform fire_place;

    private float current_damage = 0;
    ...
    private void FixedUpdate()
    {
        if (enemies_array_range != null)
        {
                    ...
                    for (int fragment_index = enemies_array_range[way].Length - 1; fragment_index >= 0; fragment_index--)
                    {
                        ...
                        if (EnemiesLogic.instance.enemies[way][fragment][0] != null)
                        {
                            if (EnemiesLogic.instance.enemies[way][fragment][0].distance_to_finish < target_dist)
                            {
                                ...
                                var new_target = EnemiesLogic.instance.enemies[way][fragment][0];
                                if (new_target != target_enemy) hide_laser();
            ...
            else if (laser.parent.gameObject.activeSelf == true) hide_laser();
        }
    }
    ...
    private void shoot_logic()
    {
        ...
        if (target_found)
        {
            if (!laser.parent.gameObject.activeSelf) laser.parent.gameObject.SetActive(true);

            if (time >= damage_interval)
            {
                time = 0;
                current_damage *= damage_coef;
                if (current_damage > max_damage) current_damage = max_damage;

                if (environment_target)
                {
                    MapInfo.instance.damage_environment(target, (int)current_damage);
                }
                else
                {
                    EnemiesLogic.instance.damage_enemy(target_enemy, (int)current_damage);
                }
            }
            time++;
            ...
            target_fire.transform.position = target;
            laser.localScale = new Vector3(1, Vector2.Distance(target, fire_place.position));
            laser.transform.position = fire_place.position;
        }
    }
    ...
    private bool old_target_found = false;
    ...
    private float get_rotation_offset(float target_rot)
    {
        ...
        if (Mathf.Abs(additional_rot1) < Mathf.Abs(additional_rot2))
        {
            ...
            {
                target_found = false;
                if (old_target_found) hide_laser();
                if (additional_rot1 >= 0) _return = rotation_speed;
                else _return = - rotation_speed;
            }
            else
            {
                target_found = true;
                _return = additional_rot1;
            }
        }
        else
        {
            if (Mathf.Abs(additional_rot2) > rotation_speed)
            {
                target_found = false;
                if (old_target_found) hide_laser();
                if (additional_rot2 >= 0) _return = rotation_speed;
                else _return = - rotation_speed;
            }
            else
            {
                target_found = true;
                _return = additional_rot2;
            }
        }
        old_target_found = target_found;
        return _return;
    }

    private void hide_laser()
    {
        laser.parent.gameObject.SetActive(false);
        time = 0;
        current_damage = start_damage;
    }
}
